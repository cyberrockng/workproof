...
| M-02 fresh checkout setup gaps | Partially fixed. `pre-build.sh` now invokes binding generation through `bash`, and README includes the nested Soldeer bootstrap commands plus the evidence commit check. |
...
| Finding | Status |
|---|---|
| H-02 multiple active TEE/ciphertext-target mismatch | Deferred as a documented P0 liveness risk. The live deployment relies on exactly one active WorkProof TEE for the extension. A production version should add explicit expected-TEE selection or allowlist/codeHash/governanceHash policy before job creation. |
...
improve the repository and should be redeployed before claiming the exact new
bytecode is live on Coston2.
